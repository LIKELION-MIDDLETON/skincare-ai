...
1. ~~`checkpoints/cnn_best.pt` 확보 방안 결정 (레포 포함 vs 외부 스토리지 다운로드)~~ →
   **결정: 레포/이미지에는 안 넣음.** 배포 서버에 파일을 직접 옮겨두고(Drive/scp 등, git 아님),
   `docker-compose.yml`에서 volume으로 컨테이너에 마운트. 서버 경로는 `CNN_WEIGHTS_PATH`
   환경변수로 지정(기본값은 저장소 안 상대경로) — 절대경로(예: `/srv/models/cnn_best.pt`)로
   두고 싶으면 `.env`나 실행 시 이 변수로 지정하면 됨. 모델 갱신 시 이미지 재빌드 없이 파일만
   교체하면 됨.

   **파일이 없을 때 동작:** 컨테이너는 정상 기동함(모델은 첫 `/analyze` 요청 때 지연 로딩).
   `/health`는 파일 유무와 상관없이 응답하되 `model_loaded: false`로 표시. 파일이 없는 상태에서
   `/analyze`를 호출하면 `FileNotFoundError`가 그대로 전파되어 500 에러가 나며, 크래시/재시작
   루프 없이 나중에 파일을 두면 재시작 없이 다음 요청부터 정상 동작함.
